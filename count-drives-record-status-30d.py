...
osTimestamp = datetime.today() - timedelta(days=15)  # number of days as a delimiter
rdTimestamp = datetime.today() - timedelta(days=30)  # number of days as a delimiter
...
# create new element for osname 9b3a
...
for index in range(len(IssueRd)):
    if '9b3a' not in IssueRd[index]['form_values']:
        ...
        if IssueRd[index]['updated_at'] >= str(rdTimestamp):  # todo: second condition
            ...
            logging.debug("OS name found and additional columns created")
        else:
...
# update records in assets according to g6 data frame 9c5d
print('update asset data for roads only')
for record in Asset:
    if '820d' in record['form_values']:
        print(''.join(record['form_values']['820d']), 'found')
        if ''.join(record['form_values']['2115']['choice_values']) in road:
            print('OS is part of the road')
            if any(x in ''.join(record['form_values']['820d']) for x in g6.osname) & any(
                            x in ''.join(record['status']) for x in
                            g6[(g6.osname == ''.join(record['form_values']['820d']))]['status']):
                print(g6[(g6.osname == ''.join(record['form_values']['820d']))]['osname'],
                      'already has status',
                      g6[(g6.osname == ''.join(record['form_values']['820d']))]['status'])
            else:
                if any(x in ''.join(record['form_values']['820d']) for x in g6.osname):
                    print(
                        (''.join(record['form_values']['820d']), "will change status from",
                         record['status']))
                    record['status'] = ''.join(
                        g6[(g6['osname'].str.contains(''.join(record['form_values']['820d'])))][
                            'status'])
                    ...
                    if ''.join(record['status']) == 'Not inspected':
                        ...
                        print(''.join(record['form_values']['820d']), 'is set to Not inspected')
...
for index in range(len(IssueOs)):
    if '9b3a' not in IssueOs[index]['form_values']:
        ...
        if IssueOs[index]['updated_at'] >= str(osTimestamp):
...
for record in Asset:
    if '820d' in record['form_values']:
        ...
        if ''.join(record['form_values']['2115']['choice_values']) in os:
            print('OS is part of the road')
            if any(x in ''.join(record['form_values']['820d']) for x in f6.osname) & any(
                            x in ''.join(record['status']) for x in
                            f6[(f6.osname == ''.join(record['form_values']['820d']))]['status']):
                print(f6[(f6.osname == ''.join(record['form_values']['820d']))]['osname'],
                      'already has status',
                      f6[(f6.osname == ''.join(record['form_values']['820d']))]['status'])
            else:
                if any(x in ''.join(record['form_values']['820d']) for x in f6.osname):
                    print(
                        (''.join(record['form_values']['820d']), "will change status from",
                         record['status']))
                    record['status'] = ''.join(
                        f6[(f6['osname'].str.contains(''.join(record['form_values']['820d'])))][
                            'status'])
                    print(('New status:', record['status']))
                    updatedRecord = fulcrum.records.update(record['id'], record)
                else:
